...
function setMonthRow(worksheet: ExcelJS.Worksheet, rowNumber: number, monthLabel: string, input: Table0ExportInput) {
  const row = worksheet.getRow(rowNumber);
...
export async function exportTable0Rpch(input: Table0ExportInput, requestedOutputPath?: string): Promise<Table0ExportResult> {
  ...
  const defaultFileName = `Таблица_0_РПЧ_${getSafeFilePart(input.station)}_${monthName}_${year}.xlsx`;
