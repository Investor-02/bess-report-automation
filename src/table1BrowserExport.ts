...
import type { ProjectReportState, ReportPeriod, StationId, Table1PaymentRecord } from './state/projectReportState';
...
export type Table1BrowserExportResult = {
  fileName: string;
...
export async function exportTable1InBrowser(input: Table1ExportInput): Promise<Table1BrowserExportResult> {
  const ExcelJSModule = await import('exceljs');
  const workbook = new ExcelJSModule.Workbook();
  const templateSource = `/templates/${encodeURIComponent(templateFileName)}?v=${Date.now()}`;
  const response = await fetch(templateSource, { cache: 'no-store' });
  if (!response.ok) {
    throw new Error('Шаблон Таблицы_1 не найден в public/templates.');
  }
  ...
  const updatedStationRows: Table1BrowserExportResult['updatedStationRows'] = [];
